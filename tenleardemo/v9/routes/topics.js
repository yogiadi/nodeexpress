...
router.post("/", middleware.isLoggedIn, function(req, res){
    ...
    var newTopic = {name: name, description: desc, author:author,date:Date.now()}
   ...
   Topic.findById(req.params.id,function(err,topic){
       if(err){
        ...
        //   "/:tutorial_id/show"
           ...
           console.log(topic.author.id);
           console.log(req.user._id);
           if (String(topic.author.id) == String(req.user._id)){
               console.log("mentor is the owner");
            req.flash("error", "Mentor is the owner");
           res.redirect("/topics/" + req.params.id);
           }else{
           if (topic.upvoteuser.indexOf(req.user._id) === -1){
            //   tutorial.priority=tutorial.priority+1;
                ...
                User.findById(topic.author.id).exec(function (err,foundUser) {
                    if(err){
                        console.log(err);
                    }else{
                       if (!foundUser.priority){
                            foundUser.priority=0;
                        }
                        foundUser.priority=foundUser.priority+1;
                        foundUser.save();
                    }
                });
                topic.save();
